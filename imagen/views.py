...
class Timeline(param.Parameterized):
    ...
    uses this objects context manager facilities to sample
    ...
    For instance, an Imagen pattern with dynamic, time-varying
    parameters may be sampled using the 'stack' method. Each snapshot
    of the pattern become a SheetView held in the returned
    SheetStack. Each of the spatial discretization in each SheetViews
    ...
    'time' parameter, set to param.Dynamic.time_fn by default. The
    returend SheetStack is then a spatiotemporal sampling of the
    pattern, i.e as a stack of images over time.
    """

    time = param.ClassSelector(default=param.Dynamic.time_fn, class_=param.Time,
        doc="""The time object shared across the time-varying objects
    ...
    def ndmap(self, obj, until, offset=0, time_density=None, value_fn = lambda obj: obj()):
        """
        ...
        the time 'until' in steps of 'time_density'.
        """
        ndmap = NdMapping(dimension_labels=['Time'])
        with self.time as t:
            t.until = until
            if time_density is not None:
                t.time_density = time_density
            obj.state_push()
            t(offset)
    ...
    def stack(self, obj, steps, offset=0,
              time_density=None, array_fn = lambda obj: obj(), **kwargs):
        """
        ...
        """
        ndmap = self.ndmap(obj, steps, offset=offset, time_density=time_density,
